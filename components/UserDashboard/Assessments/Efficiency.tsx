...
export default function Efficiency() {
  const [material, setMaterial] = useState<string>("RCC");
  const [slopeDeg, setSlopeDeg] = useState<number>(5);
  const [cleanliness, setCleanliness] = useState<string>("Clean");
  const [drainage, setDrainage] = useState<string>("Good");
  const [firstFlush, setFirstFlush] = useState<string>("Available");
  ...
  const [effValue, setEffValue] = useState<number | null>(null);
  const [userId, setUserId] = useState<string | null>(null);

  // Fetch data from Firestore on mount/auth change
  useEffect(() => {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (data.efficiencyData) {
              const saved = data.efficiencyData;
              if (saved.material) setMaterial(saved.material);
              if (saved.slopeDeg) setSlopeDeg(saved.slopeDeg);
              if (saved.cleanliness) setCleanliness(saved.cleanliness);
              if (saved.drainage) setDrainage(saved.drainage);
              if (saved.firstFlush) setFirstFlush(saved.firstFlush);
              if (saved.lastBreakdown) setLastBreakdown(saved.lastBreakdown);
              if (saved.effValue !== undefined) setEffValue(saved.effValue);
            }
          }
        } catch (error) {
  ...
  const saveToFirestore = async (data: any) => {
    ...
    try {
      ...
      await setDoc(docRef, { efficiencyData: data }, { merge: true });
    } catch (error) {
  ...
  const handleCalculate = () => {
    ...
    const newEffValue = Math.min(100, Math.max(0, rawEfficiency));

    setLastBreakdown(newBreakdown);

    // Fake premium loader for ~2.2s
    setTimeout(() => {
      setEffValue(newEffValue);
      setLoadingEff(false);
      ...
      saveToFirestore({
        ...
        drainage,
        firstFlush,
        lastBreakdown: newBreakdown,
        effValue: newEffValue,
      });
    }, 2200);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            disabled={loadingEff}
            className={`mt-4 inline-flex items-center justify-center rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-950 shadow-lg shadow-teal-900/50 bg-gradient-to-r from-cyan-400 via-teal-400 to-emerald-400 hover:from-cyan-300 hover:via-teal-300 hover:to-emerald-300 transition-all ${loadingEff ? "opacity-60 cursor-not-allowed" : ""
              }`}
          >
